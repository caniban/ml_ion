...
# Code for f0F2 parameter prediction
...
# Dropping hmF2 and TEC columns which we won't use for f0F2 predictions.
veri = veri.drop(['hmF2(t-23)', 'hmF2','TEC(t-23)','TEC' ], axis=1)
...
# Variable selection
X = np.asarray(veri_nonNaN[["DNS","DNC","HRS","HRC","f107","ap","f0F2(t-23)"]]) # INDEPENDENT VARIABLES
y = np.asarray(veri_nonNaN["f0F2"]) # DEPENDENT VARIABLE TO PREDICT
...
# Dropping hmF2 and TEC columns which we won't use for f0F2 predictions.
veri2 = veri2.drop(['hmF2(t-23)', 'hmF2', 'TEC(t-23)', 'TEC'], axis=1)
...
X_validate = np.asarray(veri2_nonNaN[["DNS","DNC","HRS","HRC","f107","ap","f0F2(t-23)"]])
y_validate = np.asarray(veri2_nonNaN["f0F2"])
...
plt.ylabel("f0F2 (Mhz)",labelpad=5, **hfont,style='italic',fontsize=14)
plt.yticks(**hfont,style='italic',fontsize=14)
plt.savefig('DT_AllYearPredictions_F.png', dpi=300)
plt.show()
...
plt.ylabel("Absolute f0F2 Deviations (MHz)",labelpad=5, **hfont,style='italic',fontsize=14)
plt.title("Decision Tree Regression", pad=10, **csfont,style='italic', fontsize=16)
ticks = [0,633,1237,1920,2519,2965,3282,3646,4089,4687,5379,6006]
labels = ["Jan 01", "Feb 01", "Mar 01", "Apr 01", "May 01", "Jun 01", "Jul 01", "Aug 01", "Sep 01", "Oct 01", "Nov 01", "Dec 01"]
...
plt.savefig('DT_AllYearDeviations_F.png', dpi=300)
plt.show()
...
plt.xlabel("Absolute f0F2 Deviations (MHz)", labelpad = 5,**hfont,style='italic',fontsize=14)
...
for fr, x, patch in zip(freq, bin_centers, patches):
    ...
    n = n+1
plt.savefig('DT_AllYearDeviationsHist_F.png', dpi=300)
...
plt.xlabel("f0F2 Predictions for 2013 (Mhz)",  labelpad=5,**hfont,style='italic',fontsize=14)
plt.ylabel("f0F2 Observations in 2013 (Mhz)", labelpad=5,**hfont,style='italic',fontsize=14)
plt.legend(loc = "lower right",prop=font)
plt.title("Decision Tree Regression", pad = 10, **csfont,style='italic', fontsize=16)
reg_xticks=[0,2,4,6,8,10,12]
reg_yticks=[0,2,4,6,8,10,12]
plt.yticks(ticks=reg_yticks,**hfont,style='italic',fontsize=14)
plt.xticks(ticks=reg_xticks,**hfont,style='italic',fontsize=14)
plt.savefig('DT_AllYearRegPlot_F.png', dpi=300)
plt.show()
